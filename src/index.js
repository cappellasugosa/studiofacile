...
Il materiale allegato è la fonte da spiegare. Analizza SOLO ciò che contiene e non inventare informazioni che non risultano dal materiale.
Spiega il contenuto in modo chiaro, didattico e piacevole, come un insegnante che accompagna lo studente nella comprensione.
Restituisci:
## SPIEGAZIONE
## CONCETTI CHIAVE
## COSA MEMORIZZARE
...
Usa esclusivamente questi titoli di sezione, uno per riga. Non usare asterischi, grassetto Markdown o blocchi di codice.`;
...
- Concludi con una sezione "## IN BREVE" e 5 punti fondamentali da ricordare.
- Usa titoli di sezione con il formato ## TITOLO. Non usare asterischi, grassetto Markdown o blocchi di codice.
...
async function flashcards(request, env) {
  const { form, text, pdf } = await materialFromForm(request);
  const n = Math.min(6, Math.max(3, Number(form.get("n")) || 6));
  if (!text?.trim() && !pdf) throw new Error("Inserisci un argomento o carica un file TXT/PDF.");

  const shortTopic = !pdf && text.trim().length < 120;
  const instruction = shortTopic
    ? `Sei StudioFacile, tutor scolastico italiano. Crea ${n} flashcard in italiano sull'argomento "${text.trim()}" usando conoscenze affidabili. Ogni carta deve avere una domanda chiara e una risposta breve ma completa. Puoi aggiungere un breve approfondimento opzionale. Restituisci SOLO JSON valido, senza Markdown, senza blocchi di codice e senza testo prima o dopo il JSON.`
    : `Sei StudioFacile, tutor scolastico italiano. Crea ${n} flashcard in italiano usando ESCLUSIVAMENTE il materiale fornito. Ogni carta deve avere una domanda chiara e una risposta breve ma completa. Non inventare informazioni. Restituisci SOLO JSON valido, senza Markdown, senza blocchi di codice e senza testo prima o dopo il JSON.`;
...
function parseFlashcardsResponse(raw, n) {
  ...
  const candidates=[];
  const add=(x)=>{x=String(x||"").trim();if(x&&!candidates.includes(x))candidates.push(x)};
  add(clean);
  const firstObj=clean.indexOf("{"); const lastObj=clean.lastIndexOf("}");
  if(firstObj>=0&&lastObj>firstObj)add(clean.slice(firstObj,lastObj+1));
  const firstArr=clean.indexOf("["); const lastArr=clean.lastIndexOf("]");
  if(firstArr>=0&&lastArr>firstArr)add(clean.slice(firstArr,lastArr+1));
  // Remove common trailing commas before JSON parsing.
  for(const c of [...candidates]) add(c.replace(/,\s*([}\]])/g,"$1"));
  let parsed=null;
  for(const candidate of candidates){try{parsed=JSON.parse(candidate);break}catch{}}
  ...
  })).filter(c=>c.question&&c.answer).slice(0,Math.min(6,n));
