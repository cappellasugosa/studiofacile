...
async function gemini(input, env, responseFormat = null) {
  ...
  try {
    ...
    throw new Error("Impossibile raggiungere Gemini in questo momento. Riprova tra poco.");
  }

  const d = await r.json();
  if (!r.ok) {
    const msg = d?.error?.message || "Errore Gemini";
    if (r.status === 429 || /quota|rate.?limit|too many requests/i.test(msg)) {
      throw new Error("Quota AI temporaneamente esaurita. Riprova quando la quota gratuita sarà nuovamente disponibile.");
    }
    if (r.status === 503 || /high demand|unavailable|overloaded/i.test(msg)) {
      throw new Error("Gemini è momentaneamente sotto forte richiesta. Riprova tra poco.");
    }
    throw new Error(msg);
  }
