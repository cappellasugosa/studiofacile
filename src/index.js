...
async function runProviderChain(input, env, section, responseFormat=null) {
  const chains={
    ...
    flashcards:["groq","openrouter","gemini"],
    "flashcards-pdf":["gemini","openrouter"]
  };
...
async function providersStatus(env){
  return json({gemini:!!env.GEMINI_API_KEY,groq:!!env.GROQ_API_KEY,openrouter:!!env.OPENROUTER_API_KEY,routing:{explain:"Gemini → Groq → OpenRouter",summary:"Gemini → OpenRouter → Groq",quiz:"Groq → OpenRouter → Gemini",flashcards:"Groq → OpenRouter → Gemini"}});
}
  ...
  async fetch(request, env) {
    ...
    try {
      if (url.pathname.startsWith("/api/")) {
        // Health/config endpoint: callable directly from the browser with GET.
        // It never exposes secret values, only whether each key is configured.
