...
const CONSENT_KEY="studiofacile-analytics-consent-v14";
...
function enableAnalytics(){
  ...
  document.getElementById("cookieBanner")?.classList.add("hidden");document.getElementById("cookieBanner")?.classList.remove("force-show");document.getElementById("cookieBanner")?.setAttribute("aria-hidden","true");
}
function rejectAnalytics(){
  localStorage.setItem(CONSENT_KEY,"denied");
  if(typeof window.gtag==="function")setConsent("denied");
  document.getElementById("cookieBanner")?.classList.add("hidden");document.getElementById("cookieBanner")?.classList.remove("force-show");document.getElementById("cookieBanner")?.setAttribute("aria-hidden","true");
}
function showPrivacySettings(){const b=document.getElementById("cookieBanner");if(!b)return;b.classList.remove("hidden");b.classList.add("force-show");b.setAttribute("aria-hidden","false");setTimeout(()=>document.getElementById("cookieAccept")?.focus(),0)}
function setupAnalyticsConsent(){
  const c=localStorage.getItem(CONSENT_KEY);
  const banner=document.getElementById("cookieBanner");
  if(c==="granted")enableAnalytics();
  else if(c==="denied")banner?.classList.add("hidden");
  else banner?.classList.remove("hidden");
  document.getElementById("cookieAccept")?.addEventListener("click",enableAnalytics);
  document.getElementById("cookieReject")?.addEventListener("click",rejectAnalytics);
  document.getElementById("privacySettings")?.addEventListener("click",showPrivacySettings);
}
const modal=document.getElementById("modal"),content=document.getElementById("content");
function applyTheme(){const dark=localStorage.getItem("studiofacile-theme")==="dark";document.body.classList.toggle("dark",dark);const b=document.getElementById("themeToggle");if(b){b.textContent=dark?"☀️":"🌙";b.title=dark?"Passa alla modalità chiara":"Passa alla modalità notte";b.setAttribute("aria-label",b.title)}}
function toggleTheme(){gaEvent("theme_toggle",{theme:document.body.classList.contains("dark")?"light":"dark"});const dark=!document.body.classList.contains("dark");localStorage.setItem("studiofacile-theme",dark?"dark":"light");applyTheme()}
