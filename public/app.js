...
function summaryUI(){content.innerHTML=`${toolHeader("summary","AI Assistant","AI ASSISTANT","Scrivi un argomento o incolla i tuoi appunti. L’AI li trasforma in spiegazioni, riassunti e materiale pronto per studiare.")}<div class="form"><textarea id="text" placeholder="Es. Rivoluzione francese, logaritmi, fotosintesi...
...
async function summary(){gaEvent("generate_summary");const raw=document.getElementById("text").value.trim();if(!raw){document.getElementById("out").innerHTML=`<div class="result">Scrivi un argomento o incolla i tuoi appunti.</div>`;return}const fd=new FormData();fd.append("text",raw);fd.append("level",document.getElementById("level").value);await send("/api/summarize",fd)}
function pdfUI(){content.innerHTML=`${toolHeader("pdf","PDF / TXT","MATERIALE","Carica un PDF o un TXT e trasformalo in un materiale di studio chiaro e ordinato.")}<div class="form">${fileUI()}<div class="form-row"><label class="field"><span>Livello</span><select id="level"><option>scuola superiore</option><option>scuola media</option><option>università</option></select></label></div><button id="pdfBtn" class="form-primary">Analizza il file <span>→</span></button><div id="out"></div></div>`;document.getElementById("pdfBtn").addEventListener("click",pdfAnalyze)}
