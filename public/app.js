...
function pomoUI(){
  ...
  const render=()=>{
    const el=document.getElementById("timer"),label=document.getElementById("pomoLabel");
    if(el)el.textContent=formatTimer();
    if(label)label.textContent=pomoRunning?"Sessione in corso":"Sessione da "+selectedMinutes+" minuti";
  };
  const syncButtons=()=>{
    const minus=document.getElementById("pomoMinus"),plus=document.getElementById("pomoPlus");
    if(minus)minus.disabled=pomoRunning||selectedMinutes<=1;
    if(plus)plus.disabled=pomoRunning||selectedMinutes>=180;
  };
  const change=(delta)=>{
    if(pomoRunning)return;
    selectedMinutes=Math.min(180,Math.max(1,selectedMinutes+delta));
    timer=selectedMinutes*60;render();syncButtons();
  };
  document.getElementById("pomoMinus").addEventListener("click",()=>change(-5));
  document.getElementById("pomoPlus").addEventListener("click",()=>change(5));
  document.getElementById("startP").addEventListener("click",startP);
  document.getElementById("resetP").addEventListener("click",resetP);
  render();syncButtons();
}
function startP(){
  if(pomoRunning||timer<=0)return;
  gaEvent("pomodoro_start");clearInterval(iv);pomoRunning=true;
  const btn=document.getElementById("startP");
  if(btn){btn.textContent="Pausa";btn.onclick=stopP;}
  document.getElementById("pomoMinus")?.setAttribute("disabled","disabled");
  document.getElementById("pomoPlus")?.setAttribute("disabled","disabled");
  const render=()=>{const el=document.getElementById("timer");if(el)el.textContent=formatTimer();const label=document.getElementById("pomoLabel");if(label)label.textContent="Sessione in corso"};
  render();
  iv=setInterval(()=>{
    timer=Math.max(0,timer-1);render();
    if(timer<=0){clearInterval(iv);iv=null;pomoRunning=false;const b=document.getElementById("startP");if(b){b.textContent="Avvia sessione";b.onclick=startP;}const label=document.getElementById("pomoLabel");if(label)label.textContent="Sessione terminata — fai una pausa";document.getElementById("pomoMinus")?.removeAttribute("disabled");document.getElementById("pomoPlus")?.removeAttribute("disabled");alert("Sessione terminata. Ora fai una pausa.")}
  },1000);
}
function stopP(){
  if(!pomoRunning)return;
  clearInterval(iv);iv=null;pomoRunning=false;
  const btn=document.getElementById("startP");if(btn){btn.textContent="Riprendi sessione";btn.onclick=startP;}
  document.getElementById("pomoMinus")?.removeAttribute("disabled");document.getElementById("pomoPlus")?.removeAttribute("disabled");
...
function resetP(){
  ...
  const btn=document.getElementById("startP");if(btn){btn.textContent="Avvia sessione";btn.onclick=startP;}
  document.getElementById("pomoMinus")?.removeAttribute("disabled");document.getElementById("pomoPlus")?.removeAttribute("disabled");
}
