...
function summaryUI(){content.innerHTML=`${toolHeader("summary","Riassumi e spiega","AI ASSISTANT","Trasforma i tuoi appunti in un materiale chiaro, ordinato e pronto per lo studio.")}<div class="form">${fileUI()}<textarea id="text" placeholder="Incolla qui il materiale da studiare..."></textarea><div class="form-row"><label class="field"><span>Livello</span><select id="level"><option>scuola superiore</option><option>scuola media</option><option>università</option></select></label></div><button id="summaryBtn" class="form-primary">Genera materiale <span>→</span></button><div id="out"></div></div>`;document.getElementById("summaryBtn").addEventListener("click",summary)}
...
let timer=1500,iv;function formatTimer(){return `${String(Math.floor(timer/60)).padStart(2,"0")}:${String(timer%60).padStart(2,"0")}`}function pomoUI(){clearInterval(iv);timer=1500;content.innerHTML=`${toolHeader("pomo","Pomodoro","FOCUS","Imposta la durata della sessione e concentrati senza distrazioni.")}<div class="pomo-panel"><div class="pomo-settings"><label class="field"><span>Durata sessione (minuti)</span><input id="pomoMin" type="number" min="1" max="180" value="25"></label></div><div class="score" id="timer">25:00</div><p id="pomoLabel">Sessione da 25 minuti</p><div class="pomo-actions"><button id="startP" class="form-primary">Avvia sessione</button><button class="secondary-btn" id="resetP">Reset</button></div></div>`;const min=document.getElementById("pomoMin");min.addEventListener("input",()=>{if(timer===1500){timer=Math.min(10800,Math.max(60,(Number(min.value)||25)*60));document.getElementById("timer").textContent=formatTimer();document.getElementById("pomoLabel").textContent=`Sessione da ${Math.round(timer/60)} minuti`}});document.getElementById("startP").addEventListener("click",startP);document.getElementById("resetP").addEventListener("click",resetP)}function startP(){gaEvent("pomodoro_start");const min=Math.min(180,Math.max(1,Number(document.getElementById("pomoMin")?.value)||25));if(timer<=0||timer===1500)timer=min*60;clearInterval(iv);iv=setInterval(()=>{timer--;const el=document.getElementById("timer");if(el)el.textContent=formatTimer();if(timer<=0){clearInterval(iv);alert("Sessione terminata. Ora fai una pausa.")}},1000)}function resetP(){clearInterval(iv);const min=Math.min(180,Math.max(1,Number(document.getElementById("pomoMin")?.value)||25));timer=min*60;const el=document.getElementById("timer");if(el)el.textContent=formatTimer();const label=document.getElementById("pomoLabel");if(label)label.textContent=`Sessione da ${min} minuti`}
...
function setupToolCarousel(){
  ...
  const refresh=()=>{
    ...
    let width=0;
    for(let i=0;i<count;i++) width+=rail.children[i].getBoundingClientRect().width;
    width+=gap*Math.max(0,count-1);
    rail.style.setProperty("--carousel-shift",`-${width}px`);
    rail.style.setProperty("--carousel-duration",`${Math.max(18,width/42)}s`);
  };
  refresh();
  window.addEventListener("resize",refresh,{passive:true});
  viewport.addEventListener("mouseenter",()=>rail.classList.add("is-paused"));
  viewport.addEventListener("mouseleave",()=>rail.classList.remove("is-paused"));
  viewport.addEventListener("touchstart",()=>rail.classList.add("is-paused"),{passive:true});
  viewport.addEventListener("touchend",()=>setTimeout(()=>rail.classList.remove("is-paused"),700),{passive:true});
}
